...
from .action import PythonAction, create_action
...
class IOConfig:
    def __init__(self, io_data: dict[str, bool]):
        self.capture: bool = io_data.get("capture", True)
...
class Task:
    ...
    def _init_deps(self, file_dep, task_dep, calc_dep):
        ...
        self.dep_changed: list[str] = []

        # file_dep
        self.file_dep: set[str] = set()
    ...
    def _init_targets(self, items: Iterable[object]) -> list[str]:
    ...
    def _init_uptodate(self, items):
        ...
        for item in items:
            ...
            if isinstance(item, bool) or item is None:
                ...
                msg = (
                    "%s. task invalid 'uptodate' item '%r'. "
    ...
    def _expand_file_dep(self, file_dep: Iterable[object]):
        ...
        for dep in file_dep:
            if isinstance(dep, str):
                ...
                msg = "%s. file_dep must be a str or Path from pathlib. " "Got '%r' (%s)"
    ...
    def init_options(self, args: list[Any] | None = None):
    ...
    def check_attr(task, attr, value, valid):
        ...
        accept = ", ".join(
            [getattr(v, "__name__", str(v)) for v in (valid[0] + valid[1])]
        )
...
def dict_to_task(task_dict):
    ...
    if "actions" not in task_dict:
        raise InvalidTask(
            "Task %s must contain 'actions' field. %s" % (task_dict["name"], task_dict)
        )
